          ...
          <div className="grid gap-6">
            <div className="surface-card p-7 sm:p-9">
              ...
              <div className="mt-6 grid gap-4 sm:grid-cols-2">
                {[
                  ["Low MOQ", "Flexible starting volumes for launch-stage cereal brands."],
                  ["Premium print", "Retail-focused graphics and finishing that look shelf-ready."],
                  ["Fast response", "Quote and spec conversations handled quickly."],
                  ["Nationwide support", "Packaging guidance and delivery planning across the USA."],
                ].map(([title, copy]) => (
                  <div key={title} className="rounded-[22px] bg-[var(--color-shell)] p-5">
                    <h3 className="text-2xl text-[var(--color-ink)]">{title}</h3>
      ...
      <section className="pb-18">
        <Container className="surface-card p-7 sm:p-9">
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-3">
            {[
              ["Understand the product", "We start with cereal type, retail environment, quantity, and target customer."],
              ["Refine structure and print", "Then we align board grade, dimensions, artwork zones, and finish direction."],
              ["Move into production", "Once specs are approved, we help transition the project into a clean quote and production path."],
            ].map(([title, copy], index) => (
              <div key={title} className="rounded-[24px] border border-[rgba(21,36,58,0.08)] bg-white p-6">
                <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-[var(--color-muted)]">
                  Step {index + 1}
                </p>
                <h3 className="mt-3 text-2xl text-[var(--color-ink)]">{title}</h3>
